"""OWT model variant with organ-balanced ROI reconstruction loss."""
...
from util.organ_balanced_loss import organ_balanced_roi_l2
...
class LossBalancedMaskedAutoencoderViT(MaskedAutoencoderViT):
    ...
    def forward_loss(self, image_target, pred, label):
        ...
        roi_loss, class_losses, class_counts, valid_samples = organ_balanced_roi_l2(
            pred,
            image_target,
            label,
            num_classes=self.model_args.num_classes_with_bg,
            background_weight=self.model_args.roi_background_weight,
        )
