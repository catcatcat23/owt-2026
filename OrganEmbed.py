...
class OrganEmbed2(nn.Module):
    ...
    def forward(self, input_x):
        ...
        # print("x.shape", x.shape) # torch.Size([64, 768, 14, 14]) # torch.Size([32, 768, 16, 14, 14])

        # B, c, d, h, w = x.shape ## 2, 128, 128, 128 ; torch.Size([64, 197, 768])
...
class SpatialRestore2(nn.Module):
    # def __init__(self, config, step_num, vis = None):
    def __init__(self, input_dim, output_dim, organ_token_total, output_hw_size):
        super(SpatialRestore2, self).__init__()

        self.conv1 = nn.Conv1d(input_dim, input_dim, 1, padding=0, bias=False)
        self.conv2 = nn.Conv1d(input_dim, output_hw_size, 1, padding=0, bias=False)
        self.conv3 = nn.Conv1d(input_dim, output_dim, 1, padding=0, bias=False)
        self.softmax = Softmax(dim=-1)
    ...
    def forward(self, input_x):
        ...
        x = x.permute(0, 2, 1)
        # print("x.shape SR1", x.shape)
        attn_ids = self.conv2(self.conv1(x)) # torch.Size([64, 200, 196])
        # attn_ids = attn_ids.permute(0, 2, 1)
        # print("attn_ids.shape SR2", attn_ids.shape) # torch.Size([64, 196, 200])
        attention_probs = self.softmax(attn_ids) # torch.Size([64, 196, 200])

        x = self.conv3(x) # torch.Size([64, 200, 512])
        x = x.permute(0, 2, 1)
        # print("x.shape2 SR", x.shape)
