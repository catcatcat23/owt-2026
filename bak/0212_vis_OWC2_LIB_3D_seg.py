...
def get_args_parser():
    ...
    parser.add_argument('--thre', type=float, default=0.5, help='0.5')
...
def save_tensor_3D_np(x, save_name, mask = False, norm=False):
    x = (x * 255).astype(np.uint8)
    x = np.expand_dims(x, axis=3)
    h,w,d,_ = x.shape
    x = np.broadcast_to(x, (h, w, d, 3))
    x_np = [x[:,:,i,:] for i in range(x.shape[2])]
    fourcc = cv2.VideoWriter_fourcc(*'mp4v') 
    out = cv2.VideoWriter(save_name+".mp4", fourcc, 20.0, (224, 224))
    for img in x_np:
        out.write(img)
