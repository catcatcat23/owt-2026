...
def gen_one_image(input_img, model, case_id=0, perceptual_loss=None):
    ...
    with torch.no_grad():
        if args.training_version.startswith('v0'):
            ...
            preds = model.unpatchify(model.forward_decoder(x_restored, cls_tokens, middle_output))
    ...
    psnr_val_thresh = psnr(target_np, pred_thresh_np, data_range=1.0)
    ssim_val_thresh = ssim(target_np, pred_thresh_np, data_range=1.0, channel_axis=0)
