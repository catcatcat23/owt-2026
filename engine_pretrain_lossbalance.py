...
def train_one_epoch(model: torch.nn.Module,
    ...
    for data_iter_step, samples in enumerate(metric_logger.log_every(data_loader, print_freq, header)): ## num_classes !=1
        ...
        if args.num_classes == 1:
            ...
            crop_applied = samples.get('crop_applied')
            crop_focus = samples.get('crop_focus_class')
            samples = image

            if args.num_classes > 1:
                image_target = image.clone()
                focus_classes = []
                if crop_focus is not None:
                    focus_classes = sorted({
                        int(value) for value in crop_focus.tolist()
                        if 0 < int(value) < args.num_classes_with_bg
                    })
                class_list = [
                    value for value in range(args.num_classes_with_bg)
                    if value not in focus_classes
                ]
                random.shuffle(class_list)
                mask_ratio = random.random() * args.mask_ratio
                selected_count = int(args.num_classes_with_bg * mask_ratio)
                selected_count = min(
                    max(selected_count, len(focus_classes)),
                    args.num_classes_with_bg - 1,
                )
                random_selected_class = focus_classes + class_list[
                    :max(0, selected_count - len(focus_classes))
                ]
        ...
        if args.num_classes > 1 and crop_applied is not None:
            ...
            metric_logger.update(
                focus_crop_fraction=(crop_focus > 0).float().mean().item()
            )
        if "LPIPS" in args.loss_version:
