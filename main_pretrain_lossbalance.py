...
def get_args_parser():
    ...
    parser.add_argument('--intensity_norm', type=str, default='per_sample',
                        ...
                        help='per_sample keeps legacy min-max; fixed_255 preserves precomputed CT windowing')
    parser.add_argument('--crop_mix_prob', type=float, default=0.0,
                        help='probability of replacing a full-image view with a local XY crop')
    parser.add_argument('--crop_focus_sample_prob', type=float, default=0.0,
                        help='within crop views, probability of sampling a row containing a focus class')
    parser.add_argument('--crop_focus_classes', type=int, nargs='*', default=[],
                        help='foreground classes prioritized for sampling and crop centering')
    parser.add_argument('--crop_size', type=int, default=0,
                        help='local crop size before model input resize; 0 uses input_size')
    parser.add_argument('--crop_jitter', type=int, default=0,
                        help='maximum XY jitter in pixels around the selected organ center')
...
if __name__ == '__main__':
    ...
    if not 0.0 <= args.crop_mix_prob <= 1.0:
        raise ValueError('--crop_mix_prob must be in [0, 1]')
    if not 0.0 <= args.crop_focus_sample_prob <= 1.0:
