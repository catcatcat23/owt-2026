"""Fair legacy OWT trainer for Common8 native-resample 448 experiments."""
...
from engine_pretrain_owt_common8_a100 import train_one_epoch
from OWT_models import mae_vit_base_patch16
...
def get_args_parser():
    parser = argparse.ArgumentParser(
        "Legacy OWT Common8 1x1x2/448 pre-training", add_help=False
    )
...
def main(args):
    ...
    if args.lambda_seg != 0:
        raise ValueError("legacy OWT has no segmentation head; --lambda_seg must be 0")
    ...
    args.slot_names = [item["name"] for item in slot_specs]
    args.slot_raw_ids = [int(item["raw_class_id"]) for item in slot_specs]
    args.model_family = "owt_legacy_fixed_token_bank"
    args.tgenc_depth = 6
    args.roi_focus_retention = (
        "legacy_batch_focus_union" if args.organ_roi_aug else "none"
    )
    ...
    print("fixed token classes: {}".format(args.slot_names))
    print("legacy TGEnc depth: {}".format(len(model.blocks2)))
    print("organ ROI augmentation: {}".format(args.organ_roi_aug))
    print("ROI focus retention: {}".format(args.roi_focus_retention))
    ...
    if misc.is_main_process():
        ...
        report = {
            ...
            "model_parameter_count_without_lpips": sum(
                parameter.numel()
                for name, parameter in model_without_ddp.named_parameters()
                if not name.startswith("perceptual_loss.")
            ),
            "trainable_parameter_count_without_lpips": sum(
                parameter.numel()
                for name, parameter in model_without_ddp.named_parameters()
                if parameter.requires_grad and not name.startswith("perceptual_loss.")
            ),
            "organ_token_total": model_args.organ_token_total,
            "tokens_per_class": model_args.token_factor,
            "tgenc_depth": len(model_without_ddp.blocks2),
            "input_size": args.input_size,
            "organ_roi_augmentation": args.organ_roi_aug,
            "roi_focus_retention": args.roi_focus_retention,
        }
        with open(output_dir / "parameter_report.json", "w", encoding="utf-8") as handle:
            json.dump(report, handle, indent=2, sort_keys=True)
        with open(output_dir / "token_class_map.json", "w", encoding="utf-8") as handle:
            json.dump(slot_specs, handle, indent=2)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        "Legacy OWT Common8 1x1x2/448 pre-training",
