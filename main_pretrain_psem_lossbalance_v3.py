...
from engine_pretrain_psem_lossbalance_v3 import train_one_epoch
...
def get_args_parser():
    ...
    parser.add_argument(
        ...
        help="Input normalization; per_sample reproduces original AbdAutoPET.",
    )
    parser.add_argument(
        "--mask_schedule",
    ...
    parser.add_argument(
        ...
        default=0,
        type=int,
        help="Use only the first N rows for smoke tests; 0 uses the full CSV.",
    )
    parser.add_argument('--positive_roi_loss_weight', type=float, default=0.25)
    parser.add_argument('--roi_positive_sample_counts', type=int, nargs='+', required=True)
...
def main(args):
    ...
    print(f"PSEM worktree: {os.getcwd()}")
    print(f"PSEM schedule: {args.mask_schedule}")
