...
from util.patch_embed import PatchEmbed3Dmae
...
class MAE3D(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_frames=16, temp_stride=2,
        ...
        # self.patch_embed = PatchEmbed3Dmae(img_size, patch_size, in_chans, embed_dim, 
        #                                 num_frames=num_frames, temp_stride=temp_stride)
        self.patch_embed = PatchEmbed3Dmae(img_size, patch_size, in_chans, embed_dim, num_frames=self.model_args.fix_frame, temp_stride=self.model_args.temp_stride)
    ...
    def forward_loss(self, imgs, pred, mask):
        ...
        image_target = self.unpatchify3D(target)
    ...
    def forward(self, imgs, mask_ratio=0.90, middle=None):
        middle_output = {}
        latent, mask, ids_restore = self.forward_encoder(imgs, mask_ratio)
        pred = self.forward_decoder(latent, ids_restore)  # (N, L, patch_size**2 *3 *2)
        loss, (p_loss) = self.forward_loss(imgs, pred, mask)

        if "LPIPS" in self.model_args.loss_version:
            middle_output["p_loss"] = p_loss
            middle_output["mask"] = mask
...
mae_vit_base_patch16 = mae3d_vit_base_patch16_dec512d4b  # decoder: 512 dim, 4 blocks
mae_vit_large_patch16 = mae3d_vit_large_patch16_dec512d4b  # decoder: 512 dim, 4 blocks
mae_vit_huge_patch14 = mae3d_vit_huge_patch14_dec512d4b  # decoder: 512 dim, 4 blocks
