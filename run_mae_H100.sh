...
ckpt=900
GPU=1
CUDA_VISIBLE_DEVICES=${GPU} python vis_OWC2_LIB_3D.py --model ${MODEL_NAME} --mask_ratio 1.0 --input_size 224 --num_classes 9 --arch_version ${ARCH_Ver} --num_classes ${CLASSES} --token_factor ${TOKEN_Fac} --training_version ${TRAIN_Ver} --loss_version ${LOSS_Ver} --output_vis ${RESULT_PATH}/${CUR_PATH}/${PROCESS_NAME}/vis/ --checkpoint ${RESULT_PATH}/${CUR_PATH}/${PROCESS_NAME}/checkpoint-${ckpt}.pth --select_cls '1' --reverse 0
