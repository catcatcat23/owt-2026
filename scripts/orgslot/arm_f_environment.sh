...
export LAMBDA_SEG=${ARM_F_LAMBDA_SEG:-0.01} LAMBDA_BG_SEG=0 SEG_SUPERVISION=retained
...
export SLOT_HEAD_TYPE=${ARM_F_HEAD_TYPE:-arm_f_attention} SLOT_HEAD_CHANNELS=128 QUERY_REFINEMENT=none PIXEL_PE=none
...
export RESUME_CHECKPOINT=${RESUME_CHECKPOINT:-} MAE_INIT_CHECKPOINT=${MAE_INIT_CHECKPOINT:-}
export BACKGROUND_REDUCTION=${BACKGROUND_REDUCTION:-mean}
case "$DIMENSION" in
  2D) export MICRO_BATCH=${ARM_F_MICRO_BATCH:-16} ACCUM_ITER=${ARM_F_ACCUM_ITER:-3} TARGET_EFFECTIVE_BATCH=192 SAVE_FREQ=10 ;;
  3D) export MICRO_BATCH=6 ACCUM_ITER=2 TARGET_EFFECTIVE_BATCH=48 SAVE_FREQ=10 FIX_FRAME=4 TEMP_STRIDE=1 ;;
  *) exit 2 ;;
esac
