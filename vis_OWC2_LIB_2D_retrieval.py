...
def read_slices_2D(args, case_id):
    """Read 2D image and label files"""
    # Load image
    img_path = os.path.join(args.load_data_vis_path, case_id+".jpg")
    img = cv2.imread(img_path)
    img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    img = torch.from_numpy(img).float()
    img = (img-img.min())/(img.max()-img.min()+0.00000001)
    img = img.permute(2, 0, 1).unsqueeze(0)  # [1, 3, H, W]

    # Load label if exists
    label_path = os.path.join(args.load_label_vis_path, case_id+".png") 
    if os.path.exists(label_path):
        label = cv2.imread(label_path)
        label = cv2.cvtColor(label, cv2.COLOR_BGR2RGB)
        label = torch.from_numpy(label).float()
        label = label.permute(2, 0, 1).unsqueeze(0)  # [1, 3, H, W]
    else:
        # label = torch.zeros_like(img[:,:1])
        exit(1)

    return {'image': img, 'label': label}

def save_image(tensor, filename, thre=0.0):
    """Save a 2D tensor as image"""
    tensor = tensor.squeeze().detach().cpu()
    if tensor.ndim == 3:  # RGB
...
def save_image_grid(tensor_list, filename, thre=0.0, label_list=None, is_list=False, print_slice=False):
    """Save multiple 2D images in a single row"""
    n = len(tensor_list)
    
    # Get image dimensions from first tensor
    h, w = tensor_list[0].squeeze().shape[-2:]
    ...
    for idx, tensor in enumerate(tensor_list):
        ...
        if label_list is not None:
            cv2.putText(grid, str(label_list[idx]), (idx*w+10, 30), 
...
if __name__ == '__main__':
    ...
    for i in range(len(case_list))[args.id_index:]:
        ...
        save_image(image_target, args.generate_in_step4+'/'+str(args.id_index)+'/'+case_id+'_test1_pred_image_top'+str(args.topk)+'_'+class_+'_target.png', thre = 0.0)  # Changed to save_image
        ...
        for c in indices_close: ## top3
            ...
            for i_b in range(inter_feature_list[c].shape[0]):
                ...
                x_masked_b = torch.unsqueeze(x_masked_b, dim=0)
                # x_masked_b = x_masked_b + 1.0 ## +1.0 bright in CNN ## +0.15 dark in Trans
                # x_masked_b = x_masked_b - 0.15 ## dark in CNN
                # x_masked_b[...,:20,:] = x_masked_b[...,:20,:] + 1.0 ## bright
                with torch.no_grad():
                    if args.training_version.startswith('v0'): ## currently only arch v11 and training v01
                        x_masked_ = model.blocks2[0](x_masked_b)
                        for bi, blk in enumerate(model.blocks2):
