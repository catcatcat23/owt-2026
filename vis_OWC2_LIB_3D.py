...
def gen_one_image(input_img, model):
    ...
    # x = tensor_like(image, device)
    # label = tensor_like(label, device)

    # x2 = tensor_like(image2, device)
    # label2 = tensor_like(label2, device)
    ...
    if args.training_version.startswith('v0'):
        ...
        # middle2 = {"image_target": image_target2, "random_selected_class": random_selected_class2}
        loss1, pred1, _ = model(x, mask_ratio=args.mask_ratio, middle=middle1)
        # loss2, pred2, _ = model(x2, mask_ratio=args.mask_ratio, middle=middle2)
        ...
        middle2 = {"image_target": x2-image_target2, "random_selected_class": _random_selected_class2}
    ...
    save_tensor_3D(pred1, args.output_vis+'/test1_pred_image_'+class_+'_'+str(args.reverse)+'.png')
    ...
    axes[0,0].imshow(pred_comb.squeeze().permute(1, 2, 0).detach().cpu().numpy())

    axes[1,0].imshow(x.squeeze().permute(1, 2, 0).detach().cpu().numpy())
    axes[1,1].imshow(x2.squeeze().permute(1, 2, 0).detach().cpu().numpy())
    axes[2,0].imshow(image_target.squeeze().permute(1, 2, 0).detach().cpu().numpy())
    axes[2,1].imshow(image_target2.squeeze().permute(1, 2, 0).detach().cpu().numpy())
...
if __name__ == '__main__':
    ...
    if args.reverse == 1:
        img1 = path+'Test/image/BDMAP_00000068/'
        label1 = path+'Test/mask/BDMAP_00000068/'
